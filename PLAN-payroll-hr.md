...
> ⚠️ **تحديث أمني لاحق (2026-09-12):** تصميم تخزين المستندات الموصوف في §3.5 وأسفل (تخزين تحت `public/uploads/` + `/uploads` في `ALWAYS_ALLOWED`) كان بيسمح لأي مستخدم مسجّل دخول — أي دور — إنه يفتح مستند أي موظف تاني (بطاقة، شهادة ميلاد) من غير أي فحص نطاق. اتصلّح: الملفات دلوقتي متخزّنة تحت `storage/uploads/` (**برّه** `public/`)، وبتتقدّم عبر `GET` محمي على نفس الـroute بيفرض نفس قاعدة `Scope` المستخدمة في باقي النظام (`canViewEmployee` في `src/lib/scope.ts`). التفاصيل والتحقق في تاريخ الـcommits بعد التاريخ ده. النصوص تحت دي فضلت زي ما هي كسجل تاريخي للقرار وقت اتخاذه.
