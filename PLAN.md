...
## 3-ب. إصلاح أمني: تسريب مستندات الموظفين ✅ (2026-09-12)

بعد دمج `PLAN-payroll-hr.md` (رفع مستندات/إقرارات الموظفين)، اكتشفنا إن أي مستخدم مسجّل دخول — بأي دور — كان يقدر يفتح مستند أي موظف تاني (بطاقة، شهادة ميلاد، رقم قومي) لأن الملفات كانت متخزّنة تحت `public/uploads/` و`/uploads` مضافة لـ `ALWAYS_ALLOWED` في الـmiddleware، يعني من غير أي فحص `Scope`.

- [x] **نقل التخزين برّه `public/`:** الملفات دلوقتي في `storage/uploads/employees/...` (خاص، `.gitignore`)، مش قابلة للوصول كملف static نهائيًا
- [x] **`GET` محمي بدل التقديم المباشر:** `GET /api/employees/:id/documents?type=...` و`.../acknowledgments?key=...` — بيتحقق من الجلسة **و** `canViewEmployee(user, id)` (نفس قاعدة `Scope`: أدمن/HR الكل، المشرف قسمه، الموظف نفسه بس) قبل ما يرجّع أي بايت. المسار الفعلي على القرص بييجي من صف الـDB، مش من الطلب
- [x] `src/lib/serialize.ts` بقى بيرجّع رابط الـAPI المحمي للعميل بدل مسار التخزين الخام
- [x] صلاحية مخصّصة `canManageEmployeeFiles` (بدل إعادة استخدام `canCorrectAttendance` لغرض تاني) لرفع/حذف الملفات (أدمن/HR)
- [x] تنضيف middleware.ts (شيل `/uploads` من `ALWAYS_ALLOWED` — بقى dead code — وتحديث التعليق القديم اللي بيقول إن الفرض في `layout.tsx`)
- [x] اختبار: `tests/employee-files-scope.test.ts` (مصفوفة الأدوار الـ4) + تحقّق فعلي end-to-end بجلسات موقّعة حقيقية (رفع → تحميل كأدمن/كالموظف نفسه/كمشرف نفس القسم = 200، كموظف تاني/كمشرف قسم تاني = 404، بلا جلسة = 401)
- [x] `tsc` + `lint` + **38/38 test** + `build` نضيفة
