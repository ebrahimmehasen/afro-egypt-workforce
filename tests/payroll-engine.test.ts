...
const NOTHING = { allowancesTotal: 0, approvedOvertimeAmount: 0, incentives: 0, bonuses: 0, paidDays: 0, deductions: [] };
...
describe("calculatePayrollRecord — salaried", () => {
  it("pays the full salary and takes off every deduction of the month, each in its own bucket", () => {
    const record = calculatePayrollRecord("PP-2026-08", {
      ...
      deductions: [ded("late", 200), ded("absence", 800), ded("early_leave", 40), ded("penalty", 300), ded("advance", 1000), ded("other", 50)],
    });
    ...
    expect(record.earlyLeaveDeduction).toBe(40);
    expect(record.penalties).toBe(300);
    expect(record.advances).toBe(1000);
    expect(record.otherDeductions).toBe(50);
    expect(record.totalDeductions).toBe(2390);
    expect(record.netSalary).toBe(13800 - 2390);
...
describe("calculatePayrollRecord — daily worker", () => {
  it("pays the paid days at the salary over 26", () => {
    const record = calculatePayrollRecord("P", { ...NOTHING, employee: DAILY_WORKER, paidDays: 22, allowancesTotal: 200 });
  ...
  it("uses the worker's own day rate when one is set", () => {
    const record = calculatePayrollRecord("P", { ...NOTHING, employee: { ...DAILY_WORKER, dailyRate: 250 }, paidDays: 10 });
