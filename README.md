...
الـmigrations **مش** بتتشغّل تلقائيًا جوّا الكونتينر — شغّل `npx prisma migrate deploy` (والزرع أول مرة) ضد قاعدة البيانات قبل ما تبعت أي traffic للكونتينر. مستندات الموظفين (بطاقات، شهادات، إقرارات) بتتخزّن **برّه** `public/` — في `/app/storage/uploads` جوّا الكونتينر (مش ملف static، بتتقدّم عبر route محمي بيتحقق من صلاحية كل مستخدم). اعمل `-v` (volume) لـ `/app/storage/uploads` عشان الملفات ما تضيعش مع كل إعادة تشغيل للكونتينر.
...
**كمان:** ضمّ فولدر `storage/uploads/` (مستندات الموظفين المرفوعة) لنفس روتين النسخ الاحتياطي — فيه بيانات حقيقية (صور بطاقات، شهادات، إقرارات موقّعة) مش موجودة في قاعدة البيانات، ومحفوظ خارج `public/` عمدًا عشان محدش يوصلّه غير عبر الـAPI المحمي.
