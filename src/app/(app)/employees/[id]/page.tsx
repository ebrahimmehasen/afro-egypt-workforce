...
import { employeeFormOptions, kindOf, loadPayContext, rulesFor, scheduleFor } from "@/lib/pay-context";
...
export default async function EmployeeProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // The daily pay breakdown, by this employee's pay type and times (all from settings), with any hand edit
  // of a posted entry shown next to what the system calculated.
  const ctx = await loadPayContext();
  const rules = rulesFor(ctx, employee);
  const schedule = scheduleFor(ctx, employee);
  const dayDates = attendance.map((a) => a.date);
  const [posted, leaves] = await Promise.all([
    dayDates.length
      ? Promise.all([
          prisma.deduction.findMany({ where: { employeeId: employee.id, autoGenerated: true, systemKey: { not: null }, date: { in: dayDates.map((d) => new Date(`${d}T00:00:00.000Z`)) } } }),
          prisma.overtime.findMany({ where: { employeeId: employee.id, autoGenerated: true, systemKey: { not: null }, date: { in: dayDates.map((d) => new Date(`${d}T00:00:00.000Z`)) } } }),
        ])
      : Promise.resolve([[], []] as const),
    prisma.leave.findMany({ where: { employeeId: employee.id, status: "approved" } }),
  ]);
  const dayRows = dayPayRows({
    employee,
    rules,
    schedule,
    days: attendance.map((a) => ({
      employeeId: employee.id,
      date: a.date,
      status: a.status,
      actualIn: a.actualIn ? new Date(a.actualIn) : null,
      actualOut: a.actualOut ? new Date(a.actualOut) : null,
      scheduledStart: new Date(a.scheduledStart),
      kind: kindOf(ctx, a.date),
    })),
    leaveFor: (date) => leaves.find((l) => dayStr(l.from) <= date && dayStr(l.to) >= date) ?? null,
    posted: [...posted[0], ...posted[1]].map((p) => ({
      systemKey: p.systemKey!,
      amount: p.amount,
      originalAmount: p.originalAmount,
      editedBy: p.editedBy,
      removed: Boolean(p.voidedAt),
    })),
  });
  const formOptions = employeeFormOptions(ctx, employee);
